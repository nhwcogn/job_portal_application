...
const JobDesc = (props:any) => {
    ...
    const cleanHTML = DOMPurify.sanitize(props.description);
     return <div className="w-2/3 bs-mx:w-full">
        ...
        <div className="flex justify-between gap-4 sm-mx:flex-wrap">
            {
                ...
                <div className="font-semibold xs-mx:text-sm">{props?props[item.id]:"NA"}{item.id=="packageOffered" && <>$</>}</div>
            </div>)
            } 
        </div>
        <Divider my="xl"/>
        ...
        <Divider my="xl"/>
        <div className="[&_h4]:text-xl [&_*]:text-mine-shaft-300 [&_li]:marker:text-bright-sun-400 [&_li]:mb-1 [&_h4]:my-5 [&_h4]:font-semibold [&_h4]:text-mine-shaft-200 [&_p]:text-justify [&_p]:text-sm [&_li]:text-sm" dangerouslySetInnerHTML={{__html:cleanHTML}}>
        </div>
        <Divider my="xl"/>
        <div>
            <div className="text-xl font-semibold mb-5">About Company</div>
            <div className="flex justify-between mb-3 items-center xs-mx:flex-wrap xs-mx:gap-2">
            <div className="flex gap-2 items-center">
